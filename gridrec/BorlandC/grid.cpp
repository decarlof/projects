...
int flag;	                                     //static
int n_det, n_ang;                                      //static
long pdim, M, M0,M02,ltbl;                             //static
float sampl, scale, L, X0,Y0;                          //static
float *SINE, *COSE, *wtbl, *dwtbl, *work, *winv;        //static
complex *cproj,*filphase, **H;                          //static

float     **G1, **G2;   /// aggiunta io
float     **S1, **S2;   /// aggiunta io
long      imgsiz;      /// aggiunta io



/*** Static function prototypes ***/

void filphase_su(long pd,float fac,                          //static
			float(*pf)(float),complex *A);              //static
void trig_su(sg_struct *SG,float **SP,float **CP);           //static
void pswf_su(pswf_struct *pswf,long ltbl,                     //static
		    long linv, float* wtbl,float* dwtbl,float* winv);
float legendre(int n,float *coefs, float x);                  //static
...
#else
//  if(G1!=G2)rel_sgram(G2);   // l'ho modificato io
//   rel_sgram(G1);   // l'ho modificato io
		...
		for(iv=vstart;iv<vfin;k++,iv++)
		  {
		    ...
		    S1[j][k]=corrn*H[iu][iv].r;
		    S2[j][k]=corrn*H[iu][iv].i;
		  }
